...
> **Stato 2026-08-26 (v1.3)**: #3 e #4 completati; #1 completato da tempo. Agunti in questa sessione: fix checksum CRC CIC (6102-famiglia + 6105), encoder Yay0, parser display list F3D→mesh (fonte gbi.h sm64 CC0), scanner blocchi MIO0/Yay0, orchestrazione splat (subprocess reale) e N64Recomp (generazione recomp.toml + run se installato). Prossimi passi naturali: serializzatore mesh→F3D per il round-trip 3D completo, MIPS disassembler (riferimento: jombo23/N64-Tools, Unlicense), avanzamento header homebrew (spec esatta da `ed64romconfig.c` libdragon).

1. **Texture viewer/decoder N64 generico** — implementare in Dart i decoder per RGBA16, RGBA32, IA16, IA8, IA4, I8, I4, CI4, CI8 usando le formule sopra (bit-shift puri, nessuna dipendenza da asset). Input: bytes forniti a runtime dal client (utente carica la propria texture estratta), output: bitmap RGBA per preview. Rispetta pienamente il vincolo — non serve mai una ROM intera, solo un blob di byte texture. ✅ *fatto (src/n64_texture.ts)*
