 ...
 * Documentazione pubblica incrociata da due fonti indipendenti:
 * ultra64.ca/files/tools/DETAILED_N64_MEMORY_MAP.txt e la voce "ROM Header"
 * della N64Brew Wiki (cross-verificata anche via ricerca pubblica per i
 * campi 0x3C-0x3F). Stesso tipo di informazione usata da ogni emulatore
 * N64 reale (es. Mupen64Plus, Ares) e da ogni flashcart per riconoscere una
 * ROM.
 */
...
export function parseN64RomHeader(rom: Uint8Array): N64RomHeader {
  ...
  return {
    ...
    imageName,
    manufacturerId: String.fromCharCode(rom[0x38]) || "?",
